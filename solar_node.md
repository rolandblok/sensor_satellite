...
| File | What |
| ---- | ---- |
| `solar_node.kicad_sch` | **the schematic** — real symbols, passes ERC |
| `solar_node.pdf` / `.svg` | rendered output, no KiCad needed to view |
| `tools/gen_schematic.py` | generates the schematic; edit this, not the `.kicad_sch` |
| `tools/scope_log.py` | logs DC measurements off the DS1054Z over LAN to CSV |
| this file | block diagram, renders on GitHub |

The Mermaid diagram below is a **block diagram**, not a schematic — it shows
connectivity and values but has no component symbols and no netlist. The KiCad
file is the authoritative version.

## Verification

The schematic is generated by script and checked by KiCad itself:
...
ERC reports **1 violation**, a benign `lib_symbol_mismatch`: the MCP1700 symbol
is a derived symbol in the KiCad library, and the generator inlines its parent's
geometry so the file is self-contained. The cached copy therefore differs from
the library copy. Nothing electrical.

Two real errors were caught this way and fixed, which is the argument for using
KiCad over a drawing tool:
